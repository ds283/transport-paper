...
void write_tasks(transport::repository<>& repo, transport::Gao_mpi<>* model)
  {
    ...
    transport::basic_range<> ks(exp(2.0), exp(12.0), 1000, transport::spacing::log_bottom);
    ...
    transport::threepf_alphabeta_task<> tk3("nonadiabatic.equilateral", ics, times, ks, alphas, betas);
    tk3.set_collect_initial_conditions(true).set_adaptive_ics_efolds(5.5);

    transport::zeta_threepf_task<> ztk3("nonadiabatic.equilateral-zeta", tk3);
